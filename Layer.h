...
public:
	...
	vector<floatptr>& resultFunc(const vector<floatptr>&); 			//calculates the output vector of the layer.
	vector<float> dsigmoid();  						//calculates the derivative of sigmoid, returns value but doesn't save
	vector<floatptr>& operator()(const vector<floatptr>& LayerInput) { return resultFunc(LayerInput); }
