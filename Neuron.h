...
public:
	...
	void activateFunc(const vector <floatptr>&); 			//Activate function, calls sigmoid
	float resultFunc(const vector <floatptr>&); 			//Calculates the neuron output, calls activateFunc
	float operator()(const vector<floatptr>& NeuronInput) { return resultFunc(NeuronInput); } //Overloading ()
